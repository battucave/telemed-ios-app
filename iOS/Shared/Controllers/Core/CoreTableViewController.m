...
- (void)showCDMAVoiceDataViewController:(NSNotification *)notification
{
	...
	if(self.storyboard && ! [settings boolForKey:@"CDMAVoiceDataDisabled"] && ! [settings boolForKey:@"CDMAVoiceDataHidden"])
	{
		...
		NSString *currentStoryboardName = [self.storyboard valueForKey:@"name"];
		
		// If currently on Main storyboard
		if([currentStoryboardName isEqualToString:@"Main"])
		{
			mainStoryboard = self.storyboard;
		}
		// If currently on MedToMed storyboard
		else if([currentStoryboardName isEqualToString:@"MedToMed"])
		{
